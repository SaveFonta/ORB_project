...
<<loading, echo=FALSE, warning=FALSE, message=FALSE>>=
library(ggplot2)
library(dplyr)
library(tidyr)
library(patchwork)
...
<<exploratory, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Bias by varying M across scenarios", fig.width=16, fig.height=20>>=
summary_tbl <- readRDS("data/unbiasedness.comprehensive.rds")

summary_tbl2 <- summary_tbl |>
  mutate(
    scenario = paste0("K = ", K, ",  tau2 = ", tau2),
    delta    = factor(delta, levels = c(0, 0.5, 1),
                      labels = c("delta = 0", "delta = 0.5", "delta = 1")) # so I plot the different deltas in the same plot
  )
...
# summary_tbl already contains bias (colMeans - true_theta), one col per method/M
bias_cols <- c(
  "full.reml", "full.pm",
  "naive.uni.reml", "naive.uni.pm",
  "naive.uni_new_reml", "naive.uni_new_pm",
  "naive.biv", "naive.biv_new",
...
# Longer 
df_bias <- summary_tbl2 |>
  dplyr::select(K, tau2, delta, scenario, all_of(bias_cols)) |>
  #long for the bias cols
  pivot_longer(cols = all_of(bias_cols), names_to = "col", values_to = "bias") |>
  # take away the M where present, NA otw
  mutate(
    M      = case_when( #new column M, if col finished for _50,. ...
      grepl("_50$",   col) ~ 50L,
      grepl("_200$",  col) ~ 200L,
      grepl("_1000$", col) ~ 1000L,
      TRUE                 ~ NA_integer_
    ), # leave only teh method name
    method = sub("_(50|200|1000)$", "", col)
  )
...
method_labels <- c(
  ...
  "naive.uni_new_reml" = "Naive Uni REML (new)",
  "naive.uni_new_pm" = "Naive Uni PM   (new)",
  "naive.biv" = "Naive Biv",
  "naive.biv_new" = "Naive Biv (new)",
  "uni.reml" = "Uni REML (old)",
  "uni.pm"= "Uni PM   (old)",
  "uni_new.reml"= "Uni REML (new)",
  "uni_new.pm" = "Uni PM   (new)",
  "biv" = "Biv      (old)",
  "biv_new"= "Biv      (new)"
)
...
df_bias <- df_bias |>
  mutate(method_label = recode(method, !!!method_labels),
         method_label = factor(method_label, levels = method_labels))



# Subset only the ones I want to compare the M 
df_bias_M.compare <- df_bias |> filter(!is.na(M))
df_bias_naive     <- df_bias |> filter(is.na(M) & !method %in% c("full.reml", "full.pm"))
 
 
 # Filter out the PM for visualisation since it is really similar to REML
 # comment next line if you want to see the results for PM
df_bias_M.compare <-  df_bias_M.compare |>
                    filter (!method_label %in% c("Uni PM   (new)", "Uni PM   (old)"))

 
# Plot with the 3 values of M on the X, the bias on y, color by delta 
p_bias_vs_M <- ggplot(df_bias_M.compare,
                      aes(x = M, y = bias, colour = delta, group = delta)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey", linewidth = 0.2) + # line at bias = 0
  geom_line(linewidth = 0.5) +
  geom_point(size = 3) +
  scale_x_continuous(breaks = c(50, 200, 1000)) +
  scale_colour_manual(values = c("delta = 0" = "purple", "delta = 0.5" = "orange", "delta = 1" = "red")) +
  facet_grid(method_label ~ scenario) + 
  labs(title    = "Bias vs M",
       x = "M (imputations)", y = "Bias", colour = NULL) +
...
<<Bias_REML, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Bias comparing different methods", fig.width=16, fig.height=20>>=

M_PLOT <- 200 # can easily change this and see results with M = 1000
 
df_plot  <- df_bias  |> 
            filter (M == M_PLOT | is.na(M))  |> 
            mutate(scenario_delta = paste0("K = ", K, ",  tau2 = ", tau2, ",  ", delta))


# Remove the lines that uses PM, only focus on REML
df_plot.reduced <- df_plot  |> 
            filter (!method_label %in% c("Full PM", "Naive Uni PM", "Naive Uni PM   (new)", "Uni PM   (old)", "Uni PM   (new)"))

df_plot.reduced |>
  ggplot(aes(x = bias, y = method_label, colour = delta, shape = delta)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey", linewidth = 0.4) +
  geom_point(size = 3) +
  scale_x_continuous(limits = c(-0.09, 0.3)) +
  scale_colour_manual(values = c("delta = 0" = "blue", "delta = 0.5" = "orange", "delta = 1" = "red")) +
  scale_y_discrete(limits = rev) +
  facet_wrap(~ scenario_delta, nrow = 4) +
  labs(title = paste0("All methods: (using M = ", M_PLOT, ")"),
       x = "Bias", y = NULL, colour = NULL, shape = NULL) +
  theme_bw(base_size = 24) +
...
<<fail.rate.1_2, echo=FALSE, warning=FALSE, message=FALSE>>=
full.simulations <- readRDS(file = "data/exploratory.check.rds")

# Errors due to the fact that impose_orb didn't create missingness
na.full.reml.prop <- mean(is.na(full.simulations$full.reml)) * 100 
na.full.reml.sum <- sum(is.na(full.simulations$full.reml)) 

# Errors due to less than impose_orb created too much missingess (less than 2 studies left) 
na.T1.reml.prop <- mean(is.na(full.simulations$uni.reml_1000)) * 100 - na.full.reml.prop
na.T1.reml.sum <- sum(is.na(full.simulations$uni.reml_1000)) - na.full.reml.sum
@

\paragraph{Results:}
We have a total of 12 scenarios, each run 1000 times. This mean that in total we have 12000 simulations.

We observe a total of \Sexpr{na.T1.reml.sum} (\Sexpr{round(na.T1.reml.prop, 2)}\% out of the total simulations) ERROR 1

And a total of \Sexpr{na.full.reml.sum} (\Sexpr{round(na.full.reml.prop, 2)}\% out of the total simulations) ERROR 2
<<tables.fail.combined, echo=FALSE, warning=FALSE, message=FALSE, results='asis'>>=
fmt_delta <- function(x) sub("\\.0$", "", format(x, trim = TRUE, scientific = FALSE))

# Build combined table: n_total shared, then type1 and type2 counts and pct
tbl1 <- full.simulations |>
  group_by(K, tau2, delta) |>
  summarise(n_total = n(),
            n_err_type1 = sum(is.na(uni.reml_1000) & !is.na(full.reml)),
            pct_type1 = round(n_err_type1 / n_total * 100, 1),
            .groups = "drop")

tbl2 <- full.simulations |>
  group_by(K, tau2, delta) |>
  summarise(n_err_type2 = sum(is.na(full.reml)),
            pct_type2 = round(n_err_type2 / n() * 100, 1),
            .groups = "drop")

combined <- tbl1 |> left_join(tbl2, by = c("K", "tau2", "delta")) |>
  mutate(delta = fmt_delta(delta),
         pct_type1 = sprintf("%.2f%%", pct_type1),
         pct_type2 = sprintf("%.2f%%", pct_type2)) |>
  dplyr::select(K, tau2, delta, n_total, n_err_type1, pct_type1, n_err_type2, pct_type2)

knitr::kable(combined, format = "latex", booktabs = TRUE,
             caption = "Proportions of Error Types 1 and 2 by scenario")
@
...
<<fail.rate.table, echo=FALSE, warning=FALSE, message=FALSE, results='asis'>>=
summary_tbl |>
  dplyr::select(K, tau2, delta, avg_Fail_Biv_M1000, avg_Fail_Biv_New_M1000) |>
  mutate(delta = fmt_delta(delta)) |>
  mutate(across(c(avg_Fail_Biv_M1000, avg_Fail_Biv_New_M1000),
                ~ sprintf("%.2f%%", . * 100))) |>
  rename(`Biv old (%)` = avg_Fail_Biv_M1000,
         `Biv new (%)` = avg_Fail_Biv_New_M1000) |>
  knitr::kable(format = "latex", caption = "Proportion of Error Number 3", booktabs = TRUE)
@
...
There is an important note, since we saw that sometimes the \texttt{impose\_orb} function can give a full dataset with no ORB or a dataset with too much ORB (less than 2 studies).
I think the best way to solve it is: 
\begin{itemize}
  \item in the case of no missingess, just report for each method the full model 
  \item in the case of too much missingness, recreate the data and reimpose the orb, maybe using a \texttt{repeat} block
\end{itemize}
...
\begin{itemize}
	...
	the off diagonals ($p_B$), or we fall back to the original formula without $\hat{\Psi}$ \footnote{Note that this problem of few studies can come from also in the univariate case for $\hat{\tau}^2$}. 
...
\section*{Appendix}
\label{appendix}
<<Bias_FULL,  echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Bias by varying M across scenarios, COMPLETE VERSION", fig.width=16, fig.height=20>>=
 df_plot |>
  ggplot(aes(x = bias, y = method_label, colour = delta, shape = delta)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey", linewidth = 0.4) +
  geom_point(size = 3) +
  scale_x_continuous(limits = c(-0.09, 0.3)) +
  scale_colour_manual(values = c("delta = 0" = "blue", "delta = 0.5" = "orange", "delta = 1" = "red")) +
    scale_y_discrete(limits = rev) + #reverse so we put FULL above
  facet_wrap(~ scenario_delta, nrow = 4) +
  labs(title = paste0("All methods: (using M = ", M_PLOT, ")"),
       x = "Bias", y = NULL, colour = NULL, shape = NULL) +
  theme_bw(base_size = 24) +
