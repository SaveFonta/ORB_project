...
\section*{Summary}
This reports what I did and what I changed in the ORB project


\subsection*{Functions}
Inside \texttt{00.functions.R} there are 9 functions
...
\subsubsection{\texttt{adj\_univariate(mi\_results, delta = 0.5, select\_type = "zscore", method.re = "REML", track.ess)}}
...
computed as $\frac{1}{\sum{w_{norm}^2}}$. The distribution of ESS could be added (or not) to the final table as a diagnostic for IS. 



\paragraph{\texttt{run\_bivariate\_imputation(data, theta\_cols, se\_cols, rho\_w = 0.4, m = 1000, new\_version = FALSE, method.re = "REML")}}
  This Generates imputations under ORB when two outcomes modeled jointly.
 ...
 When we only have a few studies, the estimate of $\Psi$ is probably super innacurate. 
	...
	the off diagonals (no $p_B$), or we fall back to the formula without $\tau^2$ \footnote{Note that this problem of few studies can come from also in the univaraite case for $\hat{tau}^2$}

\paragraph{\texttt{adj\_bivariate(mi\_results, delta = 0.5, select\_type = "zscore", method.re = "REML")}}
Applies ORB importance sampling on the imputed datasets outputted by the function \texttt{run\_bivariate\_imputation}. 

Note that I compute only one weight per imputation that is used for both outcomes


\paragraph{\texttt{generate\_bivariate\_ma(K = 12, theta = c(0.4, 0.4), tau2 = c(0.06, 0.06), rho\_b = 0.4, rho\_w = 0.4, n\_arm = 50)}}
Simulate complete bivariate meta-analysis data before ORB is imposed.





\paragraph{\texttt{impose\_orb(data, p1 = 0.4, delta\_sim = 0.5, select\_type = "zscore", orb.se = TRUE)}}
Impose ORB on Outcome 1 in simulated data.

It finallly returns a data frame with ORB-induced missingness for Outcome 1.

Note, at the beginning I wasn't sure wheter also the SEs were omitted,  so I added \texttt{orb.se}, if \texttt{TRUE}, then also se are omitted. 
...
\begin{itemize}
	...
	When we only have a few studies, the estimate of $\Psi$ is probably super innacurate. 
	I see two ways to solve this: either we fall back to \texttt{struct = "VC"} to make it easier for REML to estimate the matrix, since it doesnt have to estimate
	the off diagonals (no $p_B$), or we fall back to the original formula without $\hat{\tau}^2$ \footnote{Note that this problem of few studies can come from also in the univaraite case for $\hat{tau}^2$}. 
	My only point is that from the mathematical standpoint, including $\hat{Psi}$ (or $\hat{tau}^2$) is the correct version. Here is a small conceptual proof. 
